...
Das Orchester wohnt in `~/.claude/` und **nicht** im geprüften Projekt.
Einmal installieren, in jedem Projekt verfügbar:

```bash
git clone <dieses-repo> ~/orchester && bash ~/orchester/install.sh
```
...
`~/.claude/orchester/projekte/<projektname>/`.

Entfernen: `bash ~/orchester/install.sh --entfernen`
...
- **`playbook.md`** — Wissen über *das Prüfen selbst*: was Befunde brachte, was
  ...
  das Orchester wird also auch dadurch besser, dass du es anderswo benutzt.
...
Neue Datei unter `orchester/agents/<name>.md` in diesem Repo (bei der
