...
**Ausschließlich** unterhalb von `~/.claude/orchester/`. Nie ins geprüfte
Projekt — kein Bericht, keine Notiz, keine Konfiguration, kein Commit.
Den Projektpfad `$ARCHIV` bekommst du vom Dirigenten.
...
## 2. Playbook fortschreiben — `~/.claude/orchester/playbook.md`
...
`~/.claude/agents/` ab**. Neue feste Orchestermitglieder bestimmt der Nutzer.
