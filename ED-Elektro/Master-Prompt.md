...
| Anfrage | inquiry/lead | new entity (F4); NB: Bestellwesen already uses "Anfrage" for supplier RFQs — see the F4 naming note |
...
German plural resources under `/api/…` (`/api/angebote`, `/api/projekte`, `/api/kunden`, `/api/lieferanten`, `/api/rechnungen`, `/api/bestellungen`, `/api/offene-posten`, `/api/mitarbeiter`, `/api/abwesenheit`, `/api/urlaubsantraege`, nested sub-domains like `/api/miete/mietobjekte`), `{Domain}Controller` naming, GET collection at base path, GET/PUT/DELETE `/{id}`, nested lookups like `/projekt/{projektId}`, German action verbs as POST endpoints (`/buchen`, `/storno`), change/deletion reasons as query params (`?begruendung=…`). Filters like `Jahr`/`Monat`/`Suche` as query params. Backend port: docs disagree (8080 vs 8082) — **check `application.properties`**.
...
- **AI (Gemini):** `GeminiDokumentAnalyseService` (document analysis pattern to copy); incoming-invoice recognition: ZUGFeRD-XML parse → AI-OCR fallback (`POST /analyze-upload`, `/import-upload`); supplier auto-assignment; `POST /zugferd/extract-ai`; email `POST /beautify`; AI chat `KiHilfeController` (`/api/ki-hilfe`) with RAG (`QdrantRagService`, docker-compose Qdrant, `ai.rag.enabled`), surfaced per upstream README as a context-aware chat widget on every desktop page; Naive-Bayes spam filter (40 % rules + 60 % Bayes) on the lead pipeline; Ollama listed as optional local AI.
...
- **No per-position offer calculator.** Offers today are document-built from the `Leistung` catalog (bezeichnung, einheit, fixed `preis`) via `DocumentBuilder.tsx`; there is no calculation screen that combines articles + technician labor time (h/min) per position into a priced offer. → F7's Angebotskalkulation screen.
...
6. **Human in the loop.** Every AI-generated artifact (offer draft, triage suggestion, report draft, transcribed note) lands in a review state; an explicit human action makes it real. **AI-generated content is never auto-sent to customers or any external party.** Two clarifications: (a) internal scheduled AI reports to internal recipients — e.g. the F9b Monday report — may send automatically but must be clearly marked as AI-generated; (b) purely rule-based transactional sends with fixed templates (F8 maintenance reminders, F10.4 appointment confirmations) are ordinary features, not AI artifacts — permitted, and logged per §6.7.
...
**Additional rules:**
- **Deputy mechanism** for vacation coverage — configurable per user, time-boxed, incl. invoice approval when F&C is absent. With 11 people, every function is a single point of failure. Semantics: a deputy temporarily assumes the absent user's **complete role template** for the configured period; every action records both identities (actor + on-behalf-of); the grant is audit-logged at start and end.
- **Subcontractor representation:** subcontractor workers are `Mitarbeiter` records flagged `extern` (the expiring account from F3 attaches to that record) — so `Einsatz.mitarbeiterRef` (F2/F10), certificates (F10.9), and timesheets all hang off one principal type.
...
- Target format: weekly Ausbildungsnachweis layout rendered according to `AzubiProfil.kammer`; default new profiles to **HWK** (a Handwerksbetrieb is normally HWK); flag the choice in the plan and the manual-test script for the lead to veto — do not block the session on it.
...
**API sketch:** `BerichtsheftController` at `/api/berichtshefte` — GET `?mitarbeiterId=&jahr=`, POST `/generieren` (week + mitarbeiterId), PUT `/{id}` (edit days), POST `/{id}/einreichen`, POST `/{id}/signieren`, POST `/{id}/ablehnen?kommentar=…`, GET `/{id}/pdf`. All marked `// TODO(F3): enforce roles`. Versioning mechanism: POST `/generieren` on a week whose current version is `SIGNIERT` creates a successor version initialized from the signed version's day texts (not a fresh AI draft), status `ENTWURF`.
...
- `MeinTagPage.tsx` in the PWA (flat-pages convention, Tailwind 4): today's assignments ordered by `reihenfolge` — customer name, address (tap → navigation app), phone (tap → call), planned material (read-only from the project's Bestellwesen positions; if the codebase offers no per-project material view, show the Einsatz `notiz` only and defer material display to F10), job notes; pull-to-refresh; big-button UI (≥48 px targets, minimal text — gloves on a ladder).
...
- **Markup rule engine** (pure code, no AI): `VK = EK × factor`, factor resolved in strict order — article override → product group → EK-value tier (Staffel: cheap articles get higher percentages) → default. The optional customer-group factor (Privat/Gewerbe/Stammkunde) is applied **multiplicatively on top** of the resolved factor (`VK = EK × faktor × kundengruppenFaktor`, default 1.0) — it never participates in the resolution chain itself. **EK selection:** when multiple current supplier prices exist for an article, EK = the lowest current supplier price; the offer position stores which supplier price was used, and the PL may switch the supplier per position. **Seed values** as EK-value tiers (the discussed category names have nothing to bind to until DATANORM groups exist): EK < 50 € → 45 %, 50–500 € → 25 %, > 500 € → 12 %, default 20 % — reflecting the discussed Kleinmaterial / Rohre-Kabel / Großgeräte guidance; the company adjusts values and adds product-group rules in the admin UI (Admin/BL only, per §9) once DATANORM groups exist.
...
- **Angebotskalkulation screen — the front door for writing offers, modeled after Powerbird** (the team's current tool; see `Powerbird-Analyse.md` in this folder — the screen must feel instantly familiar to a Powerbird-trained PL):
  - New desktop page `AngebotskalkulationEditor.tsx` with a **hierarchical Titel/Gruppen position tree** (multi-level sections, Powerbird's Positionsbaum pattern) holding mixed position types: **Artikelposition** (from the DATANORM article master), **Leistungsposition** (bundle of embedded articles + Lohnminuten; nesting one level is enough), **Jumbo/Set** (prints as ONE position with ONE price while the contained articles stay tracked internally for purchasing and Nachkalkulation), **Textposition** (Textbausteine), **Zuschlagsposition**, plus `alternativ`/`eventual` flags (excluded from the sum).
  - Per-position pricing, live and entirely rule-based: material EK (lowest current supplier price, switchable per position) → F7 markup engine → VK; labor = **Lohnminuten × the rate of the position's Lohnart** (2–3 configurable Lohnarten, e.g. Meister/Geselle/Azubi, rates fed from the Verrechnungslohn baseline). A collapsible **"Preisdetails"** view shows the full calculation path per position. Pre-fill Lohnminuten from `analysiereKategorie()` hours-per-unit where history exists; the PL always overrides.
  - **Preisspiegel light:** per article, show the imported supplier prices side by side (cheapest highlighted) from the DATANORM data of multiple wholesalers — no live shop calls (IDS stays stubbed).
  - Running totals for material, labor, sum, margin — per position and per document; margin columns respect §9. Document-level discount (Dokumentrabatt). Copy positions/titles from previous offers (Stücklistenarchiv feel).
  - Data model sketch: `AngebotsKalkulation` (anfrageRef, status, dokumentRabatt, summen) + `AngebotsTitel` (kalkulationRef, parentRef nullable, bezeichnung, reihenfolge) + `AngebotsPosition` (titelRef, typ ENUM('ARTIKEL','LEISTUNG','JUMBO','TEXT','ZUSCHLAG'), artikelRef/leistungRef nullable, menge, einheit, arbeitszeitMinuten, lohnartRef, materialEk, materialVk, lohnKosten, positionsSumme, alternativ BOOLEAN, reihenfolge; JUMBO positions own child rows for their contained articles).
  - On completion the calculation generates the ANGEBOT document through the existing DocumentBuilder / `{{LEISTUNGEN_TABELLE}}` / PDF stack, attached to the Anfrage — no second PDF path; a Jumbo renders as one line. **Sollmengen handoff:** the stored positions (quantities + Lohnminuten) become the project's Soll values so the existing Vor-/Nachkalkulation compares Soll vs. Ist per position after execution — the Powerbird "Angebot als Soll-Vorgabe" loop.
  - **Catalog seeding (data, not code):** plan for importing KFE/ZVEH Leistungspositionen (Elektro: normed texts + Arbeitszeitwerte in Lohnminuten + material lists, delivered via ZVEH/Datanorm formats) and the company's own Powerbird Leistungen/old offers via **GAEB export from Powerbird → the system's existing GAEB import**. Verify the upstream GAEB import's capabilities before building anything new for this.

**Single-track rules — the company runs Powerbird today and will NOT operate two systems for the same job.** The Angebotskalkulation is built to *replace* offer writing in Powerbird, not to sit next to it:

1. **Critical path is DATA, not code.** The screen is worthless without (a) the article master with wholesaler prices (DATANORM import) and (b) Leistungen carrying Lohnminuten (KFE/ZVEH for Elektro; TGP/Arbeitswerte for SHK; plus the company's own Powerbird Leistungen via GAEB). Build order inside F7: markup engine → DATANORM import → catalog seeding → screen. Do not ship the screen against an empty catalog.
2. **Validation before cutover, time-boxed.** Re-calculate 5–10 real past Powerbird offers in the new screen and compare totals to the cent. Any deviation is an engine defect to fix, not rounding. When two consecutive offers match exactly, validation ends — that is the only period in which the same offer legitimately exists twice.
3. **Cutover by new business, never parallel operation.** From the cutover date every NEW Anfrage is calculated in ED-Elektro; inquiries and projects already running in Powerbird finish there. No offer is ever maintained in both systems.
4. **Own number range.** Offer numbers get a distinct prefix/offset that can never collide with Powerbird's numbering.
5. **One-directional handover.** Until invoicing itself moves, a won offer is handed to the commercial chain as a GAEB export plus PDF — exported once, never re-imported, never edited on both sides.
6. **Never duplicate master data.** Customers and articles have exactly one owning system per record; the other side references by number (customer number, supplier + article number) and does not maintain them.

**Acceptance:** golden-file DATANORM import test is idempotent; markup unit tests cover every resolution level and tie-breaks; changing a rule re-prices new offers but never alters existing sent offers; the Verrechnungslohn dialog reproduces the plan doc's phase-2/3 spec; all price-bearing endpoints respect §9 visibility. **Angebotskalkulation:** a PL assembles a 10-position offer (articles + labor times) without typing a single price by hand and gets a correct PDF; changing a position's minutes updates the totals live; the stored calculation reproduces the PDF positions exactly; no price value in the screen originates from an LLM.
...
9. **Subcontractor management** — BL/Admin: certificate registry per sub (§48b EStG Freistellungsbescheinigung, Betriebshaftpflicht, Kälteschein Kat. I where relevant) with expiry dates; **expired or missing certificate blocks dispatch-board planning** for that sub — an absolute block, no override (this guards the §48b Bauabzugsteuer liability); sub submits timesheets via restricted PWA → PL approves → approved hours feed post-calculation and are matched against the sub's invoice ("auf dem Zettel standen aber 12 Stunden" ends here).
...
*(build last — requires F3 roles; article search becomes genuinely useful after F7; reuses the F1/F9 AI foundations)*
...
**Architecture:**
- **One chat PIPELINE, not one controller.** Reuse the existing stack as a shared pipeline (Gemini client, `QdrantRagService`, agent loop, history entity, widget component) — but expose the assistant through a **new additive `AssistentController` at `/api/assistent`** delegating to a shared chat service. The F9f explain-mode endpoint (`/api/ki-hilfe`) stays untouched — zero upstream diff (§6.1), and the per-role 403/200 split becomes trivial.
- **Function calling over ERP data — read-only tool set v1:** `sucheAuftraege` (implemented over **Projekte** — that is what "offene Klima-Aufträge" colloquially means; derive the trade via the F4 Anfrage link on the Angebot→Projekt chain where present, otherwise return trade-unknown; do NOT add a gewerk column in F11 — if coverage turns out poor, propose it as a follow-up), `sucheAngebote` + `angebotDetails`, `sucheArtikel` (name/number → article + prices per role), `sucheBautagebuchFotos` (project/customer/date → thumbnails via the existing `DateiController`), `sucheKunden`, `offenePosten` (Admin/BL only per §9), and `sucheDokumentation` — Qdrant retrieval wrapped as just another read-only tool, so the model itself routes between data lookups and knowledge answers; with `ai.rag.enabled=false` that tool is simply absent and the assistant degrades to data-only.
- **Permission enforcement lives in the tool layer, server-side — never in the prompt.** First implementation step: read F3's enforcement code. If §9 checks sit at method/service level, pass the authenticated principal through and reuse them; if F3 put them in the web layer, build one identity-aware tool facade that re-applies the role check and the role-scoped DTO mapping on every tool call, and route all tools through it. Additionally build the tool declaration set **per role** — unpermitted tools are not even advertised to the model — and keep the server-side rejection as defense in depth.
- **Structured response contract — the model never emits URLs.** Tools return typed entity references (`{typ, id, label, thumbnailRef}`); the backend deterministically maps them to frontend routes and `DateiController` thumbnail URLs; the model's text refers to result items by index; the widget renders link/thumbnail cards from the structured part (rules before AI, §6.4 — link building is a rule).
- **Strictly read-only in v1:** the assistant answers, deep-links, and shows thumbnails; it never creates, changes, deletes, or sends anything. Write actions would be a separate, explicitly approved feature.
- **Per-user persisted conversation history:** persist user/assistant messages plus tool-call metadata only (tool name + parameters, **no cached result payloads** — tools re-execute under current permissions if context is rebuilt, so a role change never leaks stale data out of history); one history entity with a `modus` discriminator (`ASSISTENT`/`ERKLAERMODUS`) so the F9f Azubi chat gets the same DSGVO self-deletion; v1 is a single active thread per user. The F1 AI-call log stays content-free (feature, model, tokens, duration, outcome, user), so §6.7 logging survives a history deletion.
- **Entry point & config:** the upstream context-aware chat widget (present on every desktop page, §4.4) resolves its mode from role + config: tool-enabled assistant for Admin/BL/PL, F9f explain mode for Azubi, absent for Monteur/Sub. Separate config keys (`ai.assistent.enabled` + own system prompt) alongside the F9f keys, so each mode has its own kill switch.

**Acceptance:** role-scoping tests — a PL asking for company-wide financials is refused at tool level; Monteur and Sub accounts receive 403 on all `/api/assistent` endpoints; an Azubi account receives 403 on `/api/assistent` but keeps the F9f explain mode; "Zeig mir die offenen Klima-Aufträge" returns deep-link cards that resolve; photo search returns only thumbnails the caller may see; the assistant is absent with `ai.assistent.enabled=false` while F9f keeps working; every call is logged with user identity; conversation history is per-user isolated (user A can never read user B's chats) and self-deletable.
...
- Do not auto-send AI-generated content to customers or external parties (§6.6; the F9b internal report and the rule-based transactional sends of F8/F10.4 are the defined exceptions).
...
- **E2 — Automatic follow-up on unanswered offers.** `@Scheduled` job sends a friendly German follow-up at day 7 and day 14 while an offer is open, logs each touch in the F5 activity feed, stops on won/lost. Directly attacks the quote-conversion leak; pairs with the F4 loss-reason statistics. *(seen at Hero "Copilot"; note: unlike the transactional sends in F8/F10.4 this is sales outreach — if built, sending must be explicitly enabled per offer or globally by the GF)*
