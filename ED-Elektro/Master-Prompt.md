...
- **Angebotskalkulation screen — the front door for writing offers** (this is how the company actually wants to create offers: search the article yourself, enter quantity, enter how many hours/minutes the technician needs, get the offer PDF):
  - New desktop page `AngebotskalkulationEditor.tsx`: assemble an offer position by position — search articles (DATANORM data) and catalog services (`Leistung`), enter quantity, enter estimated technician labor per position in hours/minutes (pre-fill the suggestion from `analysiereKategorie()` hours-per-unit where the category has history; the PL can always override).
  - Pricing per position happens live and entirely rule-based: material via the F7 markup engine (EK → VK), labor = minutes × the Verrechnungslohn baseline rate; running totals for material, labor, sum, and margin (margin visible per §9 only).
  - Data model sketch: `AngebotsKalkulation` (anfrageRef, status, summen) + `AngebotsPosition` (kalkulationRef, typ ENUM('ARTIKEL','LEISTUNG','FREITEXT'), artikelRef/leistungRef nullable, menge, einheit, arbeitszeitMinuten, materialEk, materialVk, lohnKosten, positionsSumme, reihenfolge).
  - On completion the calculation generates the ANGEBOT document through the existing DocumentBuilder / `{{LEISTUNGEN_TABELLE}}` / PDF stack, attached to the Anfrage — no second PDF path. The stored positions (incl. estimated minutes) are kept so Nachkalkulation can later compare estimated vs. actual hours per position.
